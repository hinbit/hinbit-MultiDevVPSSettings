...
- [docs/manual_harden.md](docs/manual_harden.md)
...
After every pull, `projectctl` rechecks domain mapping and HTTPS wiring, runs `build all`, and then restarts PM2 so the running process always reflects the pulled `main` tip. A missing app-map entry is repaired during the pull run instead of surfacing as a broken deploy later.
...
Then it runs `build all` automatically on pull:
...
If the process is missing after a restart or update, `projectctl` retries the PM2 restart and smoke test once before it gives up, so a transient boot issue does not leave the domain mapped but dead.
...
- every successful UI install opens the project env popup so the operator can optionally upload a local env file before closing the install workflow
