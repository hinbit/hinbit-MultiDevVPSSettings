...
function showVpsUsage(){document.querySelectorAll('#machineList .machine').forEach((card,index)=>{card.querySelector('.vps-usage')?.remove();const usage=machines[index]?.lastStatus?.agent?.raw?.usage;if(!usage)return;const chips=card.querySelector('.machine-head .chips');if(!chips)return;const line=document.createElement('span');line.className='vps-usage';line.style.cssText='display:inline-flex;gap:4px;align-items:center;font-size:12px';line.innerHTML=usageBadge('CPU',usage.cpuPercent)+usageBadge('RAM',usage.ram?.percent)+usageBadge('DISK',usage.disk?.percent);chips.append(line)})}
