...
const VPS_CONTROL_ACTIONS = new Set(['communication-off', 'communication-on', 'process-start', 'process-stop', 'process-restart', 'pm2-suspend', 'pm2-resume', 'node-suspend', 'node-resume', 'shutdown', 'wake', 'status']);
...
function normalizeVpsWakeMachine(input, existing = {}) {
  ...
  const providerAccessKey = Object.prototype.hasOwnProperty.call(input || {}, 'providerAccessKey') && String(input.providerAccessKey || '')
...
function normalizeProviderState(value) {
  ...
  if (['off', 'stopped', 'shutdown', 'shutoff', 'powered-off', 'down'].includes(state)) return 'off';
  if (['shutting-down', 'stopping', 'terminating'].includes(state)) return 'shutting-down';
  if (['starting', 'booting', 'waking'].includes(state)) return 'starting';
...
async function checkVpsProvider(machine) {
  ...
  try {
    const data = await fetchControlJson(machine.providerEndpoint, machine.providerToken, {
      action: 'status', machineId: machine.id, resourceId: machine.providerResourceId, requestedAt: new Date().toISOString(),
    });
    return { configured: true, reachable: true, state: normalizeProviderState(data.state || data.status || data.powerState), raw: data };
  } catch (error) {
    return { configured: true, reachable: false, state: 'unknown', error: error.message };
  }
}
...
async function checkVpsWakeMachine(id) {
  ...
  const state = provider.reachable
    ? provider.state
    : (agent.reachable ? String(agent.state || 'awake') : (ssh.reachable ? 'awake' : 'offline'));
  const communication = agent.reachable ? (agent.communication === false || agent.communication === 'disabled' ? 'disabled' : 'enabled') : 'unknown';
  const boReg = { installed: Boolean(agent.reachable && agent.raw?.service === 'bo.reg'), version: String(agent.raw?.version || ''), checkedAt };
  machine.lastStatus = { checkedAt, state, communication, ssh, agent, provider, boReg };
  machines[index] = machine;
  writeVpsWakeMachines(machines);
  return sanitizeVpsMachine(machine);
}
...
async function runVpsControlAction(id, action, options = {}) {
  ...
  if ((action === 'process-start' || action === 'process-stop' || action === 'process-restart') && !processName) {
    throw new Error('PM2 process name is required');
  }
  if ((action === 'node-suspend' || action === 'node-resume') && (!Number.isInteger(pid) || pid < 1)) {
    throw new Error('Node process PID is required');
  }
  const payload = {
    action,
  ...
  if (action === 'wake') {
    ...
    result.providerConfirmation = provider;
  } else {
    if (!status.boReg?.installed || !machine.agentUrl || !machine.agentToken) throw new Error(`${action} requires a healthy installed bo.reg agent`);
    result = await fetchControlJson(`${machine.agentUrl}/v1/control`, machine.agentToken, payload);
  }
  ...
  if (result.state || result.communication !== undefined) {
    machine.lastStatus = { ...machine.lastStatus, checkedAt: payload.requestedAt, state: typeof result.state === 'string' ? result.state : (machine.lastStatus?.state || 'unknown'), communication: result.communication === false || result.communication === 'disabled' ? 'disabled' : (result.communication ? 'enabled' : machine.lastStatus?.communication || 'unknown') };
  }
  machines[index] = machine;
  writeVpsWakeMachines(machines);
...
<label>🔩 port<input id="boRegPort" value="9088"></label><label>🔩 bind host<input id="boRegBindHost" value="0.0.0.0"></label><div class="small">Install creates <code>/opt/bo.reg/.env</code> with a random token and enabled systemd service.</div>
...
function edit(m){by('machineId').value=m.id||'';fields.forEach(k=>{if(k==='policy')by(k).value=JSON.stringify(m.policy||{},null,2);else if(k==='boRegPort')by(k).value=(m.boReg||{}).port||'9088';else if(k==='boRegBindHost')by(k).value=(m.boReg||{}).bindHost||'0.0.0.0';else if(!/Password|Token|AccessKey|SecretKey/.test(k))by(k).value=m[k]||'';else by(k).value=''});by('formTitle').textContent='Edit VPS machine';by('name').focus()}
...
const awsProviderOption=document.querySelector('#provider');if(awsProviderOption&&!awsProviderOption.querySelector('option[value="aws"]')){const option=document.createElement('option');option.value='aws';option.textContent='AWS EC2';awsProviderOption.append(option)}const providerAccessInput=document.getElementById('providerAccessKey'),providerSecretInput=document.getElementById('providerSecretKey');if(providerAccessInput?.parentElement?.firstChild)providerAccessInput.parentElement.firstChild.textContent='Provider access key ';if(providerSecretInput?.parentElement?.firstChild)providerSecretInput.parentElement.firstChild.textContent='Provider secret key ';
...
function applyVpsActionAvailability(){document.querySelectorAll('#machineList .machine').forEach((card,index)=>{const machine=machines[index]||{},status=machine.lastStatus||{},agent=status.agent?.raw||{},state=status.state,communication=status.communication,installed=Boolean(status.boReg?.installed),providerReady=Boolean(status.provider?.reachable);const wake=card.querySelector('[data-act="wake"]'),shutdown=card.querySelector('[data-act="shutdown"]'),enable=card.querySelector('[data-act="communication-on"]'),disable=card.querySelector('[data-act="communication-off"]'),update=card.querySelector('[data-bo-install]');if(wake)wake.disabled=state!=='off'||!providerReady;if(shutdown)shutdown.disabled=state!=='awake'||!installed||!providerReady||agent.allowShutdown!==true;if(enable)enable.disabled=!installed||communication!=='disabled';if(disable)disable.disabled=!installed||communication!=='enabled';const installedVersion=status.boReg?.version||machine.boReg?.version||'';if(update){update.disabled=state!=='awake';if(BO_REG_GITHUB_VERSION&&versionOlder(installedVersion,BO_REG_GITHUB_VERSION)){update.style.cssText='background:#fecaca;color:#7f1d1d;border:1px solid #f87171';update.title=state==='awake'?'Update available: '+installedVersion+' -> '+BO_REG_GITHUB_VERSION:'Update requires an awake VPS'}}})}new MutationObserver(applyVpsActionAvailability).observe(document.getElementById('machineList'),{childList:true});applyVpsActionAvailability();
const vpsButtonStyle=document.createElement('style');vpsButtonStyle.textContent='button:disabled{background:#1f2937!important;color:#64748b!important;border-color:#334155!important;cursor:not-allowed!important}';document.head.append(vpsButtonStyle);function labelVpsRemove(){document.querySelectorAll('[data-delete]').forEach(button=>{button.textContent='Remove';button.classList.remove('danger');button.classList.add('ghost')})}function addNodeControls(){document.querySelectorAll('#machineList .machine').forEach((card,index)=>{const nodes=machines[index]?.lastStatus?.agent?.raw?.nodeProcesses||[];const detail=[...card.querySelectorAll('details')].find(item=>item.querySelector('summary')?.textContent.includes('Node processes'));if(!detail||detail.dataset.nodeControls)return;detail.dataset.nodeControls='1';const list=detail.querySelector('div');nodes.forEach(node=>{const row=document.createElement('div');row.className='actions';row.style.margin='6px 0';row.innerHTML='<code style="align-self:center">PID '+esc(node.pid)+' · '+esc(node.user)+' · '+esc(node.command)+'</code><button class="secondary" data-node-action="node-suspend" data-id="'+esc(machines[index].id)+'" data-pid="'+esc(node.pid)+'">Suspend</button><button class="secondary" data-node-action="node-resume" data-id="'+esc(machines[index].id)+'" data-pid="'+esc(node.pid)+'">Resume</button>';list?.append(row)})})}new MutationObserver(()=>{labelVpsRemove();addNodeControls()}).observe(document.getElementById('machineList'),{childList:true});labelVpsRemove();addNodeControls();document.addEventListener('click',async event=>{const button=event.target.closest('[data-node-action]');if(!button)return;const action=button.dataset.nodeAction;if(action==='node-suspend'&&!confirm('Suspend Node PID '+button.dataset.pid+'? It will stop until Resume is pressed.'))return;button.disabled=true;try{const result=await api('/vps-wake-machines/'+encodeURIComponent(button.dataset.id)+'/actions',{method:'POST',body:JSON.stringify({action,pid:Number(button.dataset.pid)})});message(by('listResult'),result.message||('Node '+action+' accepted'));await refreshAndCheck()}catch(error){message(by('listResult'),error.message,false)}finally{button.disabled=false}});const delayedStatusRefresh=()=>setTimeout(()=>refreshAndCheck().catch(error=>message(by('listResult'),'Post-action status refresh failed: '+error.message,false)),10000);document.addEventListener('click',event=>{if(event.target.closest('[data-act],[data-pm2-action],[data-node-action],[data-bo-install]'))delayedStatusRefresh()});setInterval(()=>refreshAndCheck().catch(error=>message(by('listResult'),'Automatic status refresh failed: '+error.message,false)),30000);
</script><script>
...
(()=>{const removeDuplicateNodeLists=()=>document.querySelectorAll('#machineList .pm2-processes').forEach(section=>[...section.querySelectorAll('details')].forEach(detail=>{if(detail.querySelector('summary')?.textContent.trim().startsWith('▸ Node processes'))detail.remove()}));const machineList=document.getElementById('machineList');if(machineList){new MutationObserver(removeDuplicateNodeLists).observe(machineList,{childList:true,subtree:true});removeDuplicateNodeLists()}})();
</script><script>
